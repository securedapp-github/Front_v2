...
function BuySale(props) {
    ...
    try {
      if (data.length > 0) {
        ...
        if (data[0].chain_id != chainId) {
          ...
          toast.error(
            `Wrong Network, Please switch to Chain_ID: "${data[0].chain_id}" `
          );
        }
      ...
      if (data[0].cliff > 0 || data[0].lockmonth > 0) {
        let enddate = data[0].end * 1000;
        for (let i = 0; i < data[0].lockmonth; i++) {
          dateTimeString[i] = new Date(
            enddate + data[0].cliff * 30000 * 86400 + i * 30000 * 86400
          );
          setreleasedate((prevItems) => [...prevItems, dateTimeString[i]]);
        }
      }

      let SALECONTRACT = new ethers.Contract(SALE, SALEABI, provider);
  ...
  const buyToken = async () => {
    try {
      ...
      setLoading(true);
      let SaleContract = new ethers.Contract(SALE, SALEABI, signerData);

      if (payaddress == "0x0000000000000000000000000000000000000000") {
        const tx = await SaleContract.buyToken(
          ethers.utils.parseUnits(buyamount.toString(), "ether"),
          { value: ethers.utils.parseUnits(buyamount.toString(), "ether") }
        );
        ...
        if (receipt.status == 1) {
          ...
          toast.success(
            "Token Approved Successfully, Please Confirm Purchase now"
          );
          const tx2 = await SaleContract.buyToken(
            ethers.utils.parseUnits(buyamount.toString(), "ether")
          );
    ...
    <>
      ...
      >
        <div className="main-body" style={{ width: "60%" }}>
          <div onClick={Change} style={{ cursor: "pointer", display: "flex" }}>
            <div className="buysale-back" style={{ paddingTop: "0.5vw" }}>
              <svg
            ...
            <div
              className="buysale-back1"
              style={{ fontSize: "1.8vw", color: "#646464" }}
            >
              {/* Go Back */}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              >
                  ...
                  >
                    <div>
                      <svg
                        ...
                        width="1.4vw"
                        height="1.4vw"
                        style={{ fill: "#00FF83", margin: "auto 0" }}
                        fill="currentColor"
                ...
                {vetted != "" && (
                  <Link
                    className="vettedLink"
                    onClick={(event) => {
                      window.open(vetted, "_blank");
                    }}
                    style={{
                      textDecoration: "none",
                      color: "white",
        ...
        <span className="new-text">{hard}{" " + token}</span> */}
              ...
              >
                ...
                <div className="expected-token">
                  {" "}
                  {price} {pay} = {decimal} {token}{" "}
                </div>
              </div>
              <div
                style={{
              ...
              <Button
                className="expected-token"
                style={{
                  border: "1px solid #00954C",
                  backgroundColor: "#00954C",
                  marginTop: "1vw",
                  padding: "10px 25px",
                }}
                onClick={() => {
                            ...
                            <td style={{ padding: "5px" }}>
                              {releaseamount[index] > 0 && hasPassed ? (
                                ...
                                <span
                                  style={{
                                    fontWeight: "bold",
                                    color: hasPassed ? "blue" : "gray",
                                  }}
                                >
                                  {hasPassed
                                    ? "-"
                                    : releaseamount[index] > 0 &&
                                      "Upcoming Claim"}
                                </span>
                              )}
                            </td>
